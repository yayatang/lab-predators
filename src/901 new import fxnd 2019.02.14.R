...
for (i in 1:length(switch_list)){
  ...
  all_data[which(all_data$sampleID==tube1 & all_data$phase==switch_phase),]$sampleID <- 'dummy1'
  all_data[which(all_data$sampleID==tube2 & all_data$phase==switch_phase),]$sampleID <- 'dummy2'
  
  all_data[which(all_data$sampleID=='dummy1'),]$sampleID <- tube2
  all_data[which(all_data$sampleID=='dummy2'),]$sampleID <- tube1
}
