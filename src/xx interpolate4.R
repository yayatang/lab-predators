...
# @@@ FUNCTION: to read meta, data, and standards raw, without cleaning------------------
get_info <- function(fileloc){
  
  # fileloc <- here::here("data/IRGA","PP-IRGA 1-1.csv") # for DEBUGGING
  # ===1: IMPORT META DATA ===
  ...
  count_check <- difftime(meta$day_flush, meta$start_day, units="days")+1
  count_check <- as.numeric(count_check)
  if(meta$incub_count != count_check){
    writeLines(paste('incubation count does not match for IRGA ', meta$sampling_num))
  } else {
      writeLines(paste('incubation count checks out! IRGA ', meta$sampling_num))
  }
  ...
  samp$incub_count <- meta$incub_count
  samp$std_time_int <- paste(meta$day_flush, samp$std_time_int)
  ...
  samp_trim$date_flush <- ymd_hms(paste(meta$day_flush, samp_trim$time_flush))
  samp_trim$date_flush <- round_date(samp_trim$date_flush, unit='minute')
  
  samp_trim$date_msre <- ymd_hms(paste(meta$day_msre, samp_trim$time_msre))
  ...
  samp_trim <- left_join(samp_trim, std_full, by = 'date_msre')
...
# file_list <- here::here('data/IRGA/PP-IRGA 1-1.csv') # debugging
...
write.csv(all_samp, file=here::here('results/1_interpolate-all_samp.csv'))
...
data_orig$samp_co2_tot <- data_orig$samp_co2_sub * co2c_const / data_orig$actual_soil
