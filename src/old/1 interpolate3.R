...
get_info <- function(fileloc){
  ...
  samp$day_flush <- meta$day_flush
  samp$day_msre <- meta$day_msre
...
make_std <- function(samp){
  ...
  std_raw$std_time_int <- ymd_hms(paste(samp$day_flush[1], std_raw$std_time_int))
  std_raw$std_time_int <- round_date(, unit='minute')
  ...
  samp2 <- left_join(samp2, std_full, by = 'date_msre')
  
  # === calculate total time in hours the tubes were incubating ===
  samp2$total_time_incub <- as.numeric(difftime(samp2$date_msre, samp2$date_flush, units="hours"))
  ...
  master <- select(samp2, sampleID, incub_count, total_time_incub, integral, inject_num, std_vector)
}
