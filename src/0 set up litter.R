# Compile tubes setup data
# November 2018
# CURRENTLY ONLY COPIED FROM EARLIER STAGE
...
tubes_target$target.moistening <- rowSums(tubes_target[,c("tube.mass", "actual.dsoil", "actual.trt", "target.water")], na.rm=TRUE)
...
write.csv(tubes_target, file = here('data/IRGA prep', 's0_tubes_target.csv'), row.names=FALSE)
...
write.csv(tubes_setup, file=here('data/IRGA prep', 's0_tubes_moistening.csv'), row.names=FALSE)
